...
export class OverlayRenderer extends Emitter implements VideoStreamRenderer {
	...
	protected crosshairs : Rectangle[] = [];
	...
	protected reset() : void {
		this.crosshairs = [{top:.5,left:.5,width:.1,height:.1},{top:.5,left:.5,width:.1,height:.1}];
	}
	...
	refresh() : Rectangle | null {
		...
		for (var i = 0; i < this.crosshairs.length; i++) {
			...
			ctx.beginPath();
			ctx.lineWidth = 1/bounds.width;
			ctx.moveTo(x, y - h);
			ctx.lineTo(x, y + h);
			ctx.stroke();
			
			ctx.beginPath();
			ctx.lineWidth = 1/bounds.height;
			ctx.moveTo(x - w, y);
			ctx.lineTo(x + w, y);
			ctx.stroke();
